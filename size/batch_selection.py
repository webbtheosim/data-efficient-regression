...
def cluster_margin(batch_size, X, chosen_idx, y_std, chosen_feat=None):
    ...
    # Perform k-medoids.
    idx = np.array([i for i in range(X_candidate.shape[0])])
    np.random.shuffle(idx)
    sample_idx = idx[0:10000]
    X_sample = X_candidate[sample_idx]
    X_sample = MinMaxScaler().fit_transform(X_sample)
    diss = squareform(pdist(X_sample))
    ...
    medoid_idx = k.medoids.tolist()
    sample_idx = sample_idx[medoid_idx].tolist()

    batch_idx = [candidate_idx[idx] for idx in sample_idx]
