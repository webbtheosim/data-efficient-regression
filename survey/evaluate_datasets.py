...
if __name__ == '__main__':
    ...
    parser.add_argument('--strategy', choices=['sf', 'sf_adaptive', 'al_adaptive', 'al'], default='sf')
    parser.add_argument('--sampler', choices=['random', 'maximin', 'medoids', 'max_entropy', 'vendi'], default='random')
    parser.add_argument('--model', choices=['nn', 'knn', 'gp', 'gp_ard', 'rf', 'xgb', 'sv'], default='nn')
    parser.add_argument('--size', type=int, default=100)
    parser.add_argument('--batch_strat', choices=['topk', 'hallucinate', 'pareto', 'cluster_margin'], default='topk')
    parser.add_argument('--seed', type=int, default=1)
    parser.add_argument('--full', action='store_true')
    parser.add_argument('--dataset_dir', default='survey-datasets/size_100')
    ...
    if args.full:
        if args.strategy == 'sf':
            filename = f'{args.task}-{args.strategy}-{args.sampler}-{args.size}-{args.seed}'
        elif args.strategy == 'sf_adaptive':
            filename = f'{args.task}-sf_adaptive-{args.size}-{args.seed}'
        elif args.strategy == 'al_adaptive':
            filename = f'{args.task}-al_adaptive-{args.sampler}-{args.model}-{args.batch_strat}-{args.size}-{args.seed}'
        else:
            filename = f'{args.task}-{args.strategy}-{args.sampler}-{args.model}-{args.batch_strat}-{args.size}-{args.seed}'
    else:
        if args.strategy == 'sf':
            filename = f'{args.task}-{args.strategy}-{args.sampler}-{args.size}-{args.seed}'
        else:
    ...
    # If applicable, get chosen features.
    if args.full and 'adaptive' in args.strategy:
        chosen_feat = np.load(f'./chosen_features/{filename}.npy')
        X = X[:,chosen_feat]
    ...
    # If space-filling on full dimensional data, get relevant features from 
    # the chosen data.
    if args.full and 'sf' in args.strategy:
        print('Filtering indices for space-filling algorithms...')
        chosen_feat = _relevant_dimensions(X_train, y_train)
        X = X[:,chosen_feat]
        X_train = X_train[:,chosen_feat]
    ...
    with open('model_results.csv', 'a') as handle:
        if args.strategy == 'sf':
            write_str = f'{args.task},{args.strategy},{args.sampler},none,none,{args.seed},{args.size}'
            for model_name in result.keys():
                for score in result[model_name]:
                    write_str = write_str + f',{score:.5f}'
            write_str = write_str + '\n'
            handle.write(write_str)
        if args.strategy == 'sf_adaptive':
            write_str = f'{args.task},{args.strategy},none,none,none,{args.seed},{args.size}'
            for model_name in result.keys():
                for score in result[model_name]:
                    write_str = write_str + f',{score:.5f}'
            write_str = write_str + '\n'
            handle.write(write_str)
        if args.strategy == 'al_adaptive':
            write_str = f'{args.task},{args.strategy},{args.sampler},{args.model},{args.batch_strat},{args.seed},{args.size}'
            for model_name in result.keys():
                for score in result[model_name]:
                    write_str = write_str + f',{score:.5f}'
            write_str = write_str + '\n'
